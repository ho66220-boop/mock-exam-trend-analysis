...
현재 데이터 기준으로는 다음 신호가 관찰되었습니다.

- 수능 이전 평균 지표 중 수학 평균과 국어 평균이 수능 결과와 강하게 연결되었습니다.
- 수능 핵심 평균 기준으로는 선형 계열 모델이 복잡한 모델보다 충분히 높은 설명력을 보였습니다.
- 상위권에서는 균형형 학생의 수능 결과가 가장 안정적으로 높았습니다.
- 중위권과 하위권에서는 일부 계층 상향 이동 사례가 확인되었습니다.
- 응시 횟수는 인과로 해석하기 어렵지만, 4-6회 보통 응시 그룹이 비교적 안정적인 결과를 보였습니다.
- 변동성이 큰 상위권 학생은 하락 위험을 함께 점검할 필요가 있습니다.
- 탐구 변경 학생은 전체적으로 탐구 유지 학생보다 수능 이전 성취와 수능 탐구 평균이 낮았으며, 변경 자체의 효과로 단정하기 어렵습니다.
- 탐구 변경 시기별 분석에서는 7월 변경 학생이 비교적 긍정적인 신호를 보였지만, 표본 수가 작아 참고용으로 해석해야 합니다.
